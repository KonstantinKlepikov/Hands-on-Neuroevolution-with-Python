...
# which use Runge-Kutta fourth-order method for numerical approximation of system
# dynamics.
#
import math
...
# m/s^2 - here negative as equations of motion
# for 2-pole system assume it to be negative
MASS_CART = 1.0   # kg
GRAVITY = -9.8
MASS_CART = 1.0   # kg
FORCE_MAG = 10.0   # N
...
def outside_bounds(x, theta1, theta2):
    ...
    return (
        x < -2.4
        or x > 2.4
        or theta1 < -THIRTY_SIX_DEG_IN_RAD
        or theta1 > THIRTY_SIX_DEG_IN_RAD
        or theta2 < -THIRTY_SIX_DEG_IN_RAD
        or theta2 > THIRTY_SIX_DEG_IN_RAD
    )
...
def rk4(f, y, dydx, tau):
    ...
    for i in range(6):
        yt[i] = y[i] + hh * dydx[i]
    # do simulation step
    x_ddot, theta_1_ddot, theta_2_ddot = calc_step(
        action=f,
        x=yt[0],
        x_dot=yt[1],
        theta1=yt[2],
