...
def run_cart_pole_simulation(net, max_bal_steps, random_start=True):
    ...
    for steps in range(max_bal_steps):
        ...
        x, x_dot, theta, theta_dot = do_step(
            action=action, x=x, x_dot=x_dot, theta=theta, theta_dot=theta_dot
        )
...
def eval_fitness(net, max_bal_steps=500000):
    ...
    if steps == max_bal_steps:
        # the maximal fitness
        return 1.0
    if steps == 0:   # needed to avoid math error when taking log(0)
        # the minimal fitness
        return 0.0
